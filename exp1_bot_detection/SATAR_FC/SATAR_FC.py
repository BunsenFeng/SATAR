...
class SemanticWord(nn.Module):
    ...
    def forward(self, text):
        ...
        if batch_size <= 8:
            ...
            while True:
                now_text = text[:, now:min(now + 8, batch_size)]
...
def train():
    ...
    TrainDataLoader = DataLoader(train_set, batch_size=28, num_workers=0, collate_fn=pad_collate,
                                 pin_memory=True, sampler=train_sampler)
    # print('DevDataLoader')
    DevDataLoader = DataLoader(validate_set, batch_size=28, num_workers=0, collate_fn=pad_collate,
                               pin_memory=True, sampler=dev_sampler)
    # print('TestDataLoader')
    TestDataLoader = DataLoader(test_set, batch_size=28, num_workers=0, collate_fn=pad_collate,
                                pin_memory=True, sampler=test_sampler)
    # print('BigDataLoader')
    #BigDataLoader = DataLoader(FullData, batch_size=32, num_workers=0, collate_fn=pad_collate,
    ...
    for i in range(EPOCH):
        ...
        if args.local_rank == 0:
            torch.save(model, 'best_models/SATARtune_epoch' + str(i) + '_acc_' + str(test_acc) + '.model')
        
        if i == smaller_step_epoch:
            optimizer = optim.SGD([param for param in model.module.fc.parameters()], lr=1e-3, momentum=0.9, weight_decay = WEIGHT_DECAY)
        # else:
        # no_up += 1
        # if no_up >= 10:
        # exit()
